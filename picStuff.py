...
#Set your own PATH
PATH = os.path.normpath('F:/GANGogh-master/misc/smallimages/')
...
for subdir, dirs, files in os.walk(root):
    ...
    for f in files:
        source = style + '\\' + f
        print((str(i) + source).encode("utf-8"))
        try:
            image = scipy.misc.imread(source)#imageio.imread(source)
            image = scipy.misc.imresize(image,(64,64))
            scipy.misc.imsave(PATH + name + '\\' + str(i) + '.png',image)
            i+=1
        except Exception:
            print(('missed it: ' + source).encode("utf-8"))
